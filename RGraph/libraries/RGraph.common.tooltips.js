...
(function (win, doc, undefined)
{
    ...
    RGraph.tooltip = function ()
    {
        ...
        function substitute (original)
        {
            ...
            (function ()
            {
                ...
                for (var i=0,str=[]; i<specific.values.length; ++i) {
                    ...
                    if (prop.tooltipsFormattedKeyColorsCss) {
                        for(property in prop.tooltipsFormattedKeyColorsCss) {
                            if (typeof property === 'string') {
                                tooltipsFormattedKeyColorsCss += '{1}: {2};'.format(property.replace(/[A-Z]/, function (match)
                                {
                                    return '-' + match.toLowerCase();
                                }), String(prop.tooltipsFormattedKeyColorsCss[property]));
                            }
                        }
                    }
            ...
            if (text.indexOf('%{table}') !== -1) {
                (function ()
                {
                    ...
                    if (prop.tooltipsFormattedTableHeaders && prop.tooltipsFormattedTableHeaders.length) {
                        str += '<thead><tr>';
                        for (var i=0; i<prop.tooltipsFormattedTableHeaders.length; ++i) {
                            str += '<th>' + prop.tooltipsFormattedTableHeaders[i] + '</th>';
                        }
                        str += '</tr></thead>';
                    }
                    ...
                    if (typeof prop.tooltipsFormattedTableData === 'object' && !RGraph.isNull(prop.tooltipsFormattedTableData)) {
                        str += '<tbody>';

                        for (var i=0; i<prop.tooltipsFormattedTableData[specific.sequentialIndex].length; ++i) {
                            str += '<tr>';
                            for (var j=0; j<prop.tooltipsFormattedTableData[specific.sequentialIndex][i].length; ++j) {
                                str += '<td>' + String(prop.tooltipsFormattedTableData[specific.sequentialIndex][i][j]) + '</td>';
                            }
                            str += '</tr>';
                        }
            ...
            if (!RGraph.isNull(args.object.properties.tooltipsPointerCss)) {
                ...
                setTimeout(function ()
                {
                    for (var i in args.object.properties.tooltipsPointerCss) {
                        if (typeof i === 'string') {
                            document.getElementById('RGraph_tooltipsPointer').style[i] = substitute(args.object.properties.tooltipsPointerCss[i]);
                        }
                    }
                }, 50);
            }
        ...
        if (args.object.properties.tooltipsPointer) {
            ...
            document.getElementById('RGraph_tooltipsPointer').style.backgroundColor = styles['background-color'];
