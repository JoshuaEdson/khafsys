...
//
    ...
    RGraph.Funnel = function (conf)
    {
        ...
        this.drawLabels = function ()
        {
            ...
            // Draws the labels
            //
            if (!RGraph.isNull(prop.labels) && typeof prop.labels === 'object' && prop.labels.length > 0) {

                var font    = prop.textFont,
                    size    = prop.textSize,
                    color   = prop.textColor,
                    labels  = prop.labels,
                    halign  = prop.textHalign == 'left' ? 'left' : 'center';
                ...
                for (var j=0; j<this.coords.length; ++j) {  // MUST be "j"
                    ...
                    if (prop.labelsSticks && labels[j]) {
                        ...
                        this.path(
                            ...
                            x + labelWidth + 10,
                            y,
                            (prop.labelsPosition === 'section' ? this.coords[j][0] + ((this.coords[j][6] - this.coords[j][0]) / 2) : this.coords[j][0]) - 10,
                            y
                        );
                    }
                }
                ...
                if (lastLabel && prop.labelsPosition === 'edge') {
        ...
        this.tooltipsFormattedCustom = function (specific, index)
        {
            ...
            if (   !RGraph.isNull(prop.tooltipsFormattedKeyColors)
                ...
                && typeof prop.tooltipsFormattedKeyColors[specific.index] === 'string')
               {
